...
component  hint="Controller for processes" output="false" extends="cfc.utils.Proxy"
{
    ...
    public any function searchSortie(required string serno, string missionid, string sortieDate, required numeric programId,required numeric locId){
        var local = {};
    ...
    private Sorties function populateSortie(required struct formRequest) {
        ...
        if (StructKeyExists(arguments.formRequest, "assignedUnitCd")) {
            local.sortie.setCurrentUnit(arguments.formRequest["assignedUnitCd"]);
        }
