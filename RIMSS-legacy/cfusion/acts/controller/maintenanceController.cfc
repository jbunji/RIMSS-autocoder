...
component extends="cfc.utils.Proxy" {
	...
	public any function insertDepotEvent (required string asset, required string loc, Repair repair, LaborPart laborPart) {
		...
		try{	
			...
			local.args.isPqdr = arguments.laborPart.getIsPqdr();
			local.args.drNum = arguments.laborPart.getDrNum();
			...
			//WO 4767 Don't insert labor part for depot event
	    	repairInsert = local.maintenanceBo.createDepotRepairUID(local.repair, local.labor);
    ...
    public void function updateEvent(required struct formRequest) {
        ...
        //writeDump(arguments.formRequest);
        //abort;
    ...
    private void function validateImpulsForm(required struct formRequest){
    	
    	if (isNull(arguments.formRequest['status']) or !len(trim(arguments.formRequest['status']))) {
	...
	public any function editEvent( string eventJob= ""){
        ...
        try{
            ...
            local.event = getEventService().getEvent(val(local.decryptEvent));
            local.asset = getAssetService().getAsset(val(local.event.getAssetId()));
            //local.qAsset = getDbUtils().getAssetsByProgram(application.sessionManager.getProgramIdSetting(),application.sessionManager.getLocIdSetting(),local.event.getAssetId());
            ...
            //addToFormRequest("highestEtm",local.qAsset.etm);
            addToFormRequest("jobId",local.event.getJobNo());
            addToFormRequest("eventId",local.event.getEventId());
            addToFormRequest("encryptedEventId",local.encryptEvent);
    ...
    public any function editEventDetail (string eventRepair= "") {
      ...
      //  dbUtils = application.objectFactory.create("DBUtils");
        ...
        try{
            ...
            local.tctoExist = dbUtils.searchTctoByRepairId(val(local.repair.getRepairId()));
            if (local.tctoExist.recordcount GT 0){
            	local.tcto = getTctoService().getTcto(val(local.tctoExist.Tcto_Id));
            	addToFormRequest("tctoId", local.tcto.getTctoId());
            	addToFormRequest("tctoNo", local.tcto.getTctoNo());
            	local.tctoEffDate = isDate(local.tcto.getEffDate()) ? UCASE(TRIM(DateFormat(local.tcto.getEffDate(),"dd-mmm-yyyy"))): local.tcto.getEffDate();
            	addToFormRequest("effDate", local.tctoEffDate);
            	local.tctoCompDate = isDate(local.tctoExist.Complete_Date) ? UCASE(TRIM(DateFormat(local.tctoExist.Complete_Date,"dd-mmm-yyyy"))): local.tctoExist.Complete_Date;
            	addToFormRequest("tctoCompDate", local.tctoCompDate);
            }
			...
			/*writeDump(getCodeService().getCode(val(local.repair.getTypeMaint())));
            abort; */           
            ...
            /*writeDump(local.repair.getHowMal());
            abort;*/
    ...
    public void function insertRepair(required struct formRequest) {
        ...
        try {
        		...
        		if ((local.pmi.getNextDueDate() NEQ "") or (local.pmi.getNextDueEtm() NEQ "")) {
        					...
        					dbUtils = application.objectFactory.create("DBUtils");
        						...
        						local.newAssetInspection = getAssetInspectionService().getAssetInspectionByRepairId(local.request.repairId);
        						    ...
        						    if ((local.pmi.getNextDueEtm() NEQ "")) {
	        ...
	        if(arguments.formrequest.bitPcId NEQ ""){
				for (i = 1; i lte listlen(form.bitPcId); i++)
				{
					currentBit = dbUtils.getBitPcByPartNo(listgetat(form.bitPcId, i));
					arguments.formRequest.bName = currentBit.noun;
					arguments.formRequest.bPartno = currentBit.partno;
					arguments.formRequest.bqty = listGetAt(form.bitQty, i);
					arguments.formRequest.laborid = repairInsert.LaborId;
			        var curlaborBitPc = populateLaborBitPc(arguments.formRequest);
			        try{
			        getLaborBitPcService().createLaborBitPc(curlaborBitPc);
	...
	public void function updateRepair(required struct formRequest) {						
        ...
        if(arguments.formrequest.bitPcId NEQ ""){
			for (i = 1; i lte listlen(form.bitPcId); i++)
			{
				...
				arguments.formRequest.bPartno = currentBit.partno;
				arguments.formRequest.bqty = listGetAt(form.bitQty, i);
				arguments.formRequest.laborid = form.laborid;
		        var curlaborBitPc = populateLaborBitPc(arguments.formRequest);
		        try{
		        getLaborBitPcService().createLaborBitPc(curlaborBitPc);
        ...
        if (StructKeyExists(arguments.formRequest, "sraPartnoId")) {
        	if (arguments.formRequest.sraPartnoId NEQ ""){
        		...
        		if (local.sraAsset.getCtAssetId() NEQ "") {
        			writeLog(file="DropDownDao" text="Here")
        			local.ctSraAsset = getInvAssetService().getInvAssets(local.sraAsset.getCtAssetId());
        			local.ctSraAsset.setSysId(local.sraPart.getCtSysId());
        		}
        	} 			
	        			...
	        			local.create.pmi = local.maintenanceBo.insertPmi(local.pmi);
	        					...
	        					if (local.nextPmiJobExist.recordcount EQ "0") {
	        						...
	        						local.newLabor = getLaborService().getLabor(local.labor.getLaborId());	
	        							...
	        							//writeLog(file="PMI" text="#local.pmi.getHistId()#");
	        						//local.newAssetInspection = getAssetInspectionService().getAssetInspection(local.pmi.getHistId());
    ...
    remote any function partOrdered(string eventId1, string repairId1, string pOAssetId1){
        ...
        try {
        	 // Clear any previous shipping info           	
	         local.asset.setTcn("");          	
	         local.asset.setShipper("");             	
	         local.asset.setShipDate(""); 
	         local.updateAsset = local.maintenanceBo.updateAsset(local.asset);        	
             partOrderInsert = local.maintenanceBo.createPartOrder(local.partOrder);
             
             
             //SEND PART ORDERED EMAIL
    ...
    remote any function partOrderedNsp(string eventId1, string repairId1, string laborId, string pOAssetId1){
    	var maintenanceBo = new MaintenanceBO();
    	local.asset = getAssetService().getAsset(val(arguments.pOAssetId1));
        var laborPartService = application.objectFactory.create("LaborPartService");
    	var partOrder = populatePartOrdered(arguments.eventId1, arguments.repairId1, arguments.pOAssetId1, local.asset.getPartnoId());
   
   		local.args.laborId = arguments.laborId;
   		local.args.remSraAssetId = arguments.pOAssetId1;
		local.laborPart = populateLaborPartRemoved(local.args);
		
        try {
        	
             laborPartInsert = local.laborpartService.createLaborPart(local.laborPart);	
             partOrderInsert = local.maintenanceBo.createPartOrder(local.partOrder);
		...
		else
		{
	   	...
	   	try{	
			...
			// Send
	   		...
	   		writeLog(file="maintenanceController" text="Email Success!");

	   		return mailService;
	   	} catch(any e){
	   		writeLog(file="maintenanceController" text="ERROR: Error sending email: #e.message#");
	   		return e.message;
	   	}
   }
   
    
    remote any function partOrderedAcknowledged(string orderId, string remAssetId){
    	var maintenanceBo = new MaintenanceBO();
    	
    	try {
    		partOrderAcknowledge = local.maintenanceBo.acknowledgePartOrder(arguments.orderId, arguments.remAssetId);
 	    } catch (any e) {
            /* TODO pass error back to the createMaintenance.cfm */
            getJavaLoggerProxy().warning(message="#getUtilities().getErrorMessage(e)#",sourceClass=getUtilities().getComponentName(this), methodName="insertRepair", user=application.sessionManager.getUsername());
            addToRequest("error", {message="Update failed #e.message#"});
...
remote any function shipRecvAcknowledged(string  orderId) returnformat="plain"{
	...
	remote any function partOrderedShipped(string  sruOrderId, string  assetId, string  shipper, string tcn, string shipDate){
        ...
        var dbUtils = application.objectFactory.create('DbUtils');
        isNSP = dbUtils.isNSP(arguments.assetId);          
    	...
    	try
			{
				...
				// If asset is an NSP then update Shipper and TCN in SRU_ORDER
				// instead of ASSET table
				if(isNSP.recordcount GT 0){
					local.sruOrderShipped.setShipper(arguments.shipper);
					local.sruOrderShipped.setTCN(arguments.tcn);
				}
	...
	remote any function partOrderedREQ(string  sruOrderId, string  assetId, string  shipper, string tcn, string shipDate){
		//This is going to be the action when setting the tracking info for shipping from the site to the depot
		//TO DO: Set the asset in transist and set the loc_idc to the DEPOT (loc_ID 338)
    	var maintenanceBo = new MaintenanceBO();
    	local.sruOrderShipped = getSruOrderService().getSruOrder(val(arguments.sruOrderId));
    	...
    	try
			{
				local.sruOrderShipped.setChgBy(application.sessionManager.getUserName());
				local.sruOrderShipped.setChgDate(Now());
				//local.sruOrderShipped.setReqShipDate(arguments.shipDate);
				//local.sruOrderShipped.setFillDate(Now());
				// If asset is an NSP then update Shipper and TCN in SRU_ORDER
				// instead of ASSET table
				//TO DO Factor in NSPs
				if(isNSP.recordcount GT 0){
					local.sruOrderShipped.setREMShipper(arguments.shipper);
					local.sruOrderShipped.setREMTCN(arguments.tcn);
					local.sruOrderShipped.setREMSruShipDate(arguments.shipDate);
				}
				sruOrderShip = local.maintenanceBo.sruShipPart(local.sruOrderShipped);
				// Get repair record location to set assets destination location
				local.repair = getRepairService().getRepair(sruOrderShip.getRepairId());
				local.event = getEventService().getEvent(local.repair.getEventId());
				//Get the partno_id from the part_list via the asset
				local.asset = getAssetService().getAsset(arguments.assetId);
				local.partList = getPartListService().getPartList(local.asset.getPartNoId());
				// Only update ASSET table if part IS NOT an NSP
				if(NOT isNSP.recordcount GT 0){
					partOrderShip = local.maintenanceBo.shipPartOrder(arguments.assetId, arguments.shipper, arguments.tcn, arguments.shipDate, local.partList.getLocIDR());
				}
            ...
            //getJavaLoggerProxy().warning(message="#getUtilities().getErrorMessage(e)#",sourceClass=getUtilities().getComponentName(this), methodName="insertRepair", user=application.sessionManager.getUsername());
            addToRequest("error", {message="Update failed #e.message#"});
         	dumpLocation = expandPath("RIMSSDebug.html");

		 	WriteDump(var="#e.detail#",format='html', output=dumpLocation);
    ...
    private Event function populateEvent(required struct formRequest) {
        ...
        if (StructKeyExists(arguments.formRequest, "eventId")) {
            //local.event.setEventId(arguments.formRequest["eventId"]);
            local.event = getEventService().getEvent(arguments.formRequest["eventId"]);
        }
    ...
    public any function searchTcto (string searchCriteria=""){   	        	
        ...
        {        	         	
		   ...
		   WriteDump(var="#local.search#", format='html', output=dumpLocation);
        }
        catch(Any e)
        {
        	writeoutput("#e#");
