...
<RIMSS:layout section="inventory" subSection="#application.sessionManager.getSubSection(true)#">
    <RIMSS:subLayout/>
		...
		<table class="two_column_table" cellpadding="0px" cellspacing="0px" style="">
							...
							<select class="select_field font9" id="status_select" name="status_select" <cfif in_transit EQ "Y">disabled=disabled</cfif>>  
								<cfoutput>                                
			                        <cfloop array="#aStatus#" index="idx">
			                            <option value="#idx[3]#" <cfif status_cd eq idx[1] >selected=selected</cfif>>#idx[2]#</option>
			                        </cfloop>
								</cfoutput>
		                    </select>
							<cfoutput>
			...
			<tr>
				<td class="column button_container" colspan="2" style="text-align:center">
					<input type="submit" name="btnUpdate" id="btnUpdate" value="UPDATE" onclick="setAction('edit.inventory',this);setMethod('doAction',this);"> 
