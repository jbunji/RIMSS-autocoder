...
<RIMSS:layout section="inventory" subSection="#application.sessionManager.getSubSection()#">
	...
	<cfset systemArray = arrayNew(1)>
	
	<!---Creating array containing all SYSTEMS for location --->
	<cfoutput query="qSystems">
		<cfset arrayAppend(systemArray,qSystems.system)>
	</cfoutput>
	
	<!---If SYSTEM is not set in SESSION... --->
	<cfif !structKeyExists(session, "system")>
		<cfif arrayContains(systemArray,"POD")>
			<!---Setting default system to POD if location has Pod assets --->
			<cfparam name="session.system" default="POD" >
		<cfelse>
			<!---Setting default system to first system in array if location does not have Pod assets --->
			<cfparam name="session.system" default="#systemArray[1]#"> 
		</cfif>
	<cfelseif arrayContains(systemArray, session.system)>
		<!---If SYSTEM is set in SESSION and location has assets for that system, keep the default system the same --->
		<cfset session.system = #session.system#>
	<cfelse>
		<!---If SYSTEM is set in SESSION and location does not have assets for that system, set default system to first system in array --->
		<cfset session.system = #systemArray[1]#>
	</cfif>
	
	<!---Setting default system in URL variable--->
