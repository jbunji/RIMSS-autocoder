...
function showBitPcDialog(obj) {
    var dialogLookup = $('#bitPcLookup'); 

    //show dialog
    var d = {};
    if($('#bitPcId').length > 0){
        d.bitPcId = $.trim($('#bitPcId').val());
    }
...
function setupBitPcDialog() {
    ...
    if($(dialogLookup).length <= 0) {
        $(document.createElement('div')).attr({'id':'bitPcLookup'}).hide().appendTo('body');
        dialogLookup = $('#bitPcLookup'); 
    }
    ...
    //show dialog
    $('a#bitpc_lookup_ref').click(function(event) {
        var title = "Bit Piece";
        ...
        function getCheckedValues(){
            ...
            $('input.loadedBitPc:checked').each(function() {
        ...
        function addTable(){
            $('#bitPcContainer').empty();
            if ($(".loadedBitPc:checked").length) {
                var tableHTML = '<table class="currentBitPc globalTable"><thead><tr><th>PartNo</th><th>Noun</th><th>Qty</th></tr></thead><tbody></tbody></table>';
                $("#bitPcContainer").attr("align","center").html(tableHTML);
...
function updateTable(){
            addTable();
            var table = $('#bitPcContainer table.currentBitPc');
            var rowsHTML = "";
            $(".loadedBitPc:checked").each(function(event){
                 var partno = $(this).parents("tr").find(".partno").text();
                 //alert(partNo);
                 var noun = $(this).parents("tr").find(".noun").text();
                 var id = $(this).prop("id");

                 rowsHTML += '<tr><td>'+ partno +'</td><td>'+ noun +'</td><td><input name="bitQty" type="text" value="1"></td></tr>';
            ...
            try {
                   var dt = $('#bitPcContainer table.currentBitPc').dataTable({ 
                      "sDom":"t"
                    });  
