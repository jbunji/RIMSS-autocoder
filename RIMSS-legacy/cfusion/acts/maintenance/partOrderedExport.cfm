			...
			<form id="searchPartsOrderedResults" name="searchPartOrderedResults" method="post" action="index.cfm">
			   ...
			   <table class="globalTable" id="partOrderedTable">
			   		<tr bgcolor="#CCCCCC">
			   			...
			   			<td align="center" colspan="1" style="border-left-width:3px;"><b>ACKNOWLEDGE</b></td>
				   ...
				   <cfloop condition="#rc.qSearch.next()#"> 
				        <tr class="<cfif rc.qSearch.getCursor() mod 2> odd <cfelse> even </cfif>" id="#HTMLEditFormat(trim(rc.qSearch.getOrderId()))#">
							...
							<td>
								<cfif HTMLEditFormat(trim(rc.qSearch.getREQ_Tcn())) NEQ ''>
									<cfswitch expression="#rc.qSearch.getREQShipper()#">
										<cfcase value="FEDEX">
											#HTMLEditFormat(trim(rc.qSearch.getREQ_Tcn()))#
											<span style="font-size:8px;padding-left:5px;">(#HTMLEditFormat(trim(rc.qSearch.getREQ_Shipper()))#)</span>
										</cfcase>
										<cfcase value="UPS">
											#HTMLEditFormat(trim(rc.qSearch.getREQ_Tcn()))#
											<span style="font-size:8px;padding-left:5px;">(#HTMLEditFormat(trim(rc.qSearch.getREQ_Shipper()))#)</span>
							...
							<td>#DateFormat(HTMLEditFormat(trim(rc.qSearch.getREQ_SHIP_DATE())),"dd-mmm-yyyy")#</td>
...
<cfscript>
	filePath = getTempDirectory() & fileName;
	 ...
	 table = CreateObject("java", "com.lowagie.text.pdf.PdfPTable").init(13);
	 ...
	 cell = CreateObject("java", "com.lowagie.text.pdf.PdfPCell").init(CreateObject("java","com.lowagie.text.Phrase").init("SHIP DATE", header));
	 cell.setBackgroundColor(color.gray);
	 table.addCell(cell);	 
	...
	for(i=1; i LTE rc.search.recordcount;i++){
		...
		if(rc.search.REQ_TCN[i] != ''){
			toDepotShipperTCN = rc.search.REQ_TCN[i] & ' (' & rc.search.REQ_SHIPPER[i] & ')';
		}
		
		if(rc.search.TCN[i] != ''){
			toSiteShipperTCN = rc.search.TCN[i] & ' (' & rc.search.SHIPPER[i] & ')';
		}		
	   	...
	   	table.addCell(CreateObject("java", "com.lowagie.text.pdf.PdfPCell").init(CreateObject("java","com.lowagie.text.Phrase").init(HTMLEditFormat(toDepotShipperTCN), body)));	   		   
	   	table.addCell(CreateObject("java", "com.lowagie.text.pdf.PdfPCell").init(CreateObject("java","com.lowagie.text.Phrase").init(DateFormat(HTMLEditFormat(trim(rc.search.REQ_SHIP_DATE[i])),"dd-mmm-yyyy"), body)));	   	
	   	...
	   	table.addCell(CreateObject("java", "com.lowagie.text.pdf.PdfPCell").init(CreateObject("java","com.lowagie.text.Phrase").init(HTMLEditFormat(toSiteShipperTCN), body)));
   }
