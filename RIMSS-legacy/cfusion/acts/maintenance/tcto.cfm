		...
		try {
            ...
            $(document).ready(function(){
				...
				//13 = enter key is pressed
				      if (event.which == 13) {	
						//RWR attempt to fix broken search
				      	//RWR value is undefined many times... Need to fix cf that sets value	
				      	var validateText = window.document.getElementById('searchCriteria').value;				      	
				      	//alert('Enter Key SearcCreteria value:' + validateText);
				      	//Now I get a valid value here, but some where in the process it is invalid..
				      	if(validateTest =="undefined")
				      	{
...
<form id="maintStatusForm">
	...
	<cfif isDefined("rc.qSearch")>
	 ...
	 <!-- RWR debug lines only--->  
	 <!---
	<cfdump var="#REQUEST#"/>
	   <cfdump var="#server#" label="Server Scope">
	   <cfdump var="#QoQSearch#" abort >
		--->
			
		<div class="mainContent">  
			<form id="searchBacklogResults" name="searchBacklogResults sticky-headers" method="post" action="index.cfm">
			...
			<cfif QoQSearch.recordcount GT 0>			
			   ...
			   <cfoutput query="QoQSearch" > 
			       ...
			       <tr>
					...
					<!---	
					<tr><td>prev Config #prevConfig#</td></tr>
...
<div style="display:none">
	...
	<table id="tctoTableExport">
	   ...
	   <tbody> 	 		
	   		...
	   		<cfif prevConfig NEQ #QoQSearch.config#>
	   			<tr>
	   				<!---"Hidden" columns below are neccessary for DataTables render row properly for Excel export due to "colspan" property --->
