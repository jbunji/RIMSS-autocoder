...
<RIMMS:layout layout="acts">
	...
	<script type="text/javascript">
		...
		$(function() {
						...
						$.post(
			              url,
			              {
						   ...
						   pOAssetId1: id
								...
								dialog.dialog("close");
								$(".nspLabel" + id).html("(A part was successfully ordered.)");
								});
			...
			// JJP 02/08/18 This is now deprecated. Only worked for 1 NSP
			/*$("#requestNsp").click(function(event){
				var obj = $(this).closest("td");
				var id = obj.data("id");
				var partno = obj.find('[name="nspPartno"]').val();
				var noun = obj.find('[name="nspNoun"]').val();
				var nspAssetId = obj.find('[name="nspAssetId"]').val(); 
				
				
				$('#nspLabel').html(partno + " " + noun);
				
				$("#orderNspDialog").data("id", nspAssetId).dialog("open");
			});*/
		});
		
		// JJP 02/08/18 Allows multiple NSPs to have Parts ordered
		function NSPFunc(id,partno,noun,nspAssetId)
		{		
				$('#nspLabel').html(partno + " " + noun);
	...
	<div id="orderNspDialog" style="margin: 0 auto; text-align:center" title="Non Serialized Part Request">
			<p style="padding: 10px 0">Are you sure you want to order a Non Serialized Part for </p>
			<span id="nspLabel"></span>?
	</div> 
						                 ...
						                 <cfoutput>
						                   <cfloop query="rc.qLaborBitPc">    
						                        <tr class="<cfif rc.qLaborBitPc.currentRow mod 2> odd <cfelse> even </cfif>">
						                            ...
						                            <td data-id="#rc.qLaborBitPc.LABOR_BIT_ID#">
						                            	#HTMLEditFormat(trim(BIT_PARTNO))#
						                            	<!---<input type="hidden" name="nspPartno" value="#rc.qLaborBitPc.BIT_PARTNO#">
						                            	<input type="hidden" name="nspNoun" value="#rc.qLaborBitPc.bit_name#">
						                            		<input type="hidden" name="nspAssetId" value="#rc.qLaborBitPc.NSP_ASSET_ID#">--->
						                            	<cfif rc.qLaborBitPc.is_nsp EQ 'Y'>
						                            		<cfif rc.qLaborBitPc.has_part_order EQ 'N'>
						                            			<span class="nspLabel#rc.qLaborBitPc.NSP_ASSET_ID#"><input type="button" name="requestNsp" value="REQUEST A PART FROM DEPOT" id="requestNsp#rc.qLaborBitPc.NSP_ASSET_ID#" class="menubuttons middle1" onclick="NSPFunc('requestNsp#rc.qLaborBitPc.NSP_ASSET_ID#',#rc.qLaborBitPc.BIT_PARTNO#,'#rc.qLaborBitPc.bit_name#',#rc.qLaborBitPc.NSP_ASSET_ID#);" onfocus="if(this.blur)this.blur()" /></span>
						                            		<cfelseif rc.qLaborBitPc.has_part_order EQ 'Y'>
						                            			<span class="nspLabel">(A part has already been ordered.)</span>
						                            		</cfif>	
						                            	</cfif>
...
<!---				<cfif StructKeyExists(rc, "qLaborBitPc") AND rc.qLaborBitPc.recordcount NEQ 0>
                ...
                <table class="one_column_table" cellpadding="0" cellspacing="0">
                    <tbody>
						...
						<tr>
							<td class="column" style="text-align:center;"><cfoutput><a href="index.cfm?action=edit.maintenance&eventJob=#eventid#" >Return to Update Maintenance</a></br>
								<a href="controller.cfm?action=viewp5failure&repairid=<cfoutput>#eventRepair#</cfoutput>&parentSerNo=<cfoutput>#form.serno#</cfoutput>">Generate P5 Maintenance Failure Report</cfoutput></td>
						</tr>
                    </tbody>
                </table>
            </div>
