...
component displayname="DBUtils" hint="A list of utility functions for query the database" {
    ...
    public numeric function getCurrentCfgMetersSequenceNumber(required string assetId) {
        ...
        var qry = new Query();
        var qryResult = "";
        var sql = "select max(meter_seq) as meter_seq " & 
                  "from core_tables.cfg_meters " &
                  "where (asset_id, eff_date) = (select asset_id, max(eff_date) " &
                  "                              from core_tables.cfg_meters " &
                  "                              where asset_id = ? " &
                  "                              group by asset_id)";
   ...
   public query function searchSortie(required string serno, string missionid, string sortieDate, required numeric programId,required numeric locId){
		...
		local.sql &="	ORDER BY gs.sortie_date desc nulls last, gs.mission_id, gs.serno, gcu.code_value, gcrange.code_value, gcactype.code_value, gs.ac_tailno, gs.ac_station, gs.sortie_effect, gs.remarks, gs.reason  ";
   ...
   public query function searchNonPoddedSortie(required string serno, string missionid, string sortieDate, required numeric programId,required numeric locId){
		...
		AND IS_NON_PODDED = 'Y'
		AND (ASSIGNED_UNIT IN (SELECT UNIT_CD FROM GLOBALEYE.LOCATION WHERE LOC_ID = :locId) OR CURRENT_UNIT IN (SELECT UNIT_CD FROM GLOBALEYE.LOCATION WHERE LOC_ID = :locId)) 	
		...
		local.sql &="	ORDER BY s.sortie_date desc nulls last, s.mission_id, s.serno, range.code_value, ac.code_value, s.ac_tailno, s.ac_station, s.sortie_effect, s.remarks, s.reason  ";
   ...
   public query function searchConfiguration(required string criteria,required numeric programId,required numeric locId,string system='AIRBORNE'){
       ...
       local.sql = "SELECT a.asset_id sra_asset_id,
				...
				/*AND a.loc_ida = :locId added in due to work order 4635 redmine issue 870*/
				/* 
				Food for thought check for a super user account to allow to search for all locations.
				application.sessionManager.getUserModel().isMemberOfGroup("PCS_SU")
				*/
       ...
       local.regExCriteria = (ARGUMENTS.criteria neq "")?Replace(UCASE(ARGUMENTS.criteria)," ", "|", "All" ):"$|";
   	   ...
   	   local.sql = "
                     ...
                     (SELECT CT_ASSET_ID, GE_ASSET_ID, SUM(DELTA) DELTA
                        FROM (  
                            SELECT CM.ASSET_ID CT_ASSET_ID, 
                                   ...
                                   (MAX(NVL(VALUE_OUT, VALUE_IN)) - LEAST(MIN(VALUE_IN), MIN(NVL(VALUE_OUT, VALUE_IN)))) DELTA
               ...
               WHERE a.partno_id = p.partno_id
                 ...
                 AND a.active = 'Y'    
                 AND (a.loc_ida = :unit OR a.loc_idc = :unit)
       ...
       local.qryResult = "";
        ...
        local.sql = "
			 SELECT DISTINCT(a.partno_id),
			  p.config,
			  ROUND((select count(1) from globaleye.asset ac, globaleye.code c where ac.partno_id = a.partno_id and c.code_id = ac.status_cd and c.code_value = 'FMC' AND ac.LOC_IDC = :unit AND ac.active = 'Y')/
              (select count(1) from globaleye.asset ac where ac.partno_id = a.partno_id AND ac.LOC_IDC = :unit AND ac.active = 'Y') * 100,2) FMC_PCT,
			  (select count(1) from globaleye.asset ac where ac.partno_id = a.partno_id AND ac.LOC_IDC = :unit AND ac.active = 'Y')  TOTAL,
			  (select count(1) from globaleye.asset ac, globaleye.code c where ac.partno_id = a.partno_id and c.code_id = ac.status_cd and c.code_value = 'FMC' AND ac.LOC_IDC = :unit AND ac.active = 'Y' )  FMC,
			  (select count(1) from globaleye.asset ac, globaleye.code c where ac.partno_id = a.partno_id and c.code_id = ac.status_cd and c.code_value = 'PMC' AND ac.LOC_IDC = :unit AND ac.active = 'Y' )  PMC,
			  (select count(1) from globaleye.asset ac, globaleye.code c where ac.partno_id = a.partno_id and c.code_id = ac.status_cd and c.code_value = 'NMCM' AND ac.LOC_IDC = :unit AND ac.active = 'Y')  NMCM,
			  (select count(1) from globaleye.asset ac, globaleye.code c where ac.partno_id = a.partno_id and c.code_id = ac.status_cd and c.code_value = 'NMCS' AND ac.LOC_IDC = :unit AND ac.active = 'Y')  NMCS,
			  (select count(1) from globaleye.asset ac, globaleye.code c where ac.partno_id = a.partno_id and c.code_id = ac.status_cd and c.code_value = 'CNDM' AND ac.LOC_IDC = :unit AND ac.active = 'Y')  CNDM,
			  (select count(1) from globaleye.asset ac, globaleye.code c where ac.partno_id = a.partno_id and c.code_id = ac.status_cd and c.code_value = 'UNKN' AND ac.LOC_IDC = :unit AND ac.active = 'Y')  UNKN			
			FROM globaleye.asset a,
			  globaleye.part_list p,
...
/* WriteDump(local.sql);*/
 ...
 /*  if (arguments.criteria EQ ""){
	...
	*/
/*   public query function getSpares1(required string program, string noun=""){
    var local = {};
       local.nounClause = "";
       //local.partnoCodeService = application.objectFactory.create("PartnoCodeService");
       //local.partnoIdList = local.partnoCodeService.getPartnoIdAsValueListByProgram(val(ARGUMENTS.program));
  ...
  */
   public query function searchSpares(required string program,string criteria="",string noun=""){
    var local = {};
...
/*UPPER(TRIM(p.noun)) like :criteria 
	...
	*/
      ...
      /*
     	try
         {
		 	...
		 	WriteDump(var="#local.qryResult#", format='html', output=dumpLocation);
		 	//WriteDump(var="#ucase(trim(application.sessionManager.getSubSection()))#", format='html', output=dumpLocation);
		 	//WriteDump(var="#local.qry#",format='html', output=dumpLocation);
		 	WriteOutput("<p>#local.sql#</p>");
         	WriteOutput("<p>#ucase(trim(application.sessionManager.getSubSection()))#</p>");
         }
         catch(Any e)
         {
         	WriteOutput("#e#");
   ...
   public query function searchPmi(required string program,string criteria="", required string loc, string dueDateInterval="60"){
    var local = {};
...
(
           ...
           from globaleye.event e,   
                (select er.event_id, rr.repair_id, rr.repair_seq, ai.pmi pmi_type, ai.next_etm_is_due, c.description, NVL2(rr.how_mal, cm.code_value||' - '||cm.description, NULL) how_mal, ll.corrective, ll.remarks
           ...
           and GA.CT_ASSET_ID = cumHrs.ct_asset_id(+))";
			if (arguments.dueDateInterval EQ "") {
				local.sql = local.sql & "where (((start_job - sysdate) <= 60) or ((next_etm_is_due - delta) <= 200))";
			} else {
				local.sql = local.sql & "where (((start_job - sysdate) <= :dueDateInterval) or ((next_etm_is_due - delta) <= 200))";
			}
			local.sql = local.sql & "AND(
           ...
           if (arguments.criteria EQ ""){
           	 local.sql = local.sql & "              
                         and stop_job is null 
                         order by start_job desc, pod_serno, jobno, repair_seq";
           } else {
           	   local.sql = local.sql & "
           	             order by start_job desc, pod_serno, jobno, repair_seq";
           }				
       ...
       local.qry.addParam(name="loc", value="#val(trim(arguments.loc))#",cfsqltype="CF_SQL_NUMERIC");    
	   local.qry.addParam(name="regExCriteria", value="#ucase(trim(local.regExCriteria))#",cfsqltype="CF_SQL_VARCHAR");  
	   local.qry.addParam(name="dueDateInterval", value="#trim(arguments.dueDateInterval)#",cfsqltype="CF_SQL_NUMERIC");  
	...
	*/
    ...
    var local = {};
                ...
                (                   
                       ...
                       from globaleye.event e,                
                            ...
                            (SELECT r.event_id,
	                               ...
	                               r.repair_seq,
	                               r.narrative,
	                               NVL (c.description, 0) type_maint,
	                               l.labor_id,
	                               lp.labor_part_id,
	                               lp.asset_id sra_asset,
	                               a.serno     sra_serno,
	                               pl.partno   sra_partno,
	                               pl.noun     sra_noun,
	                               l.corrective,
	                               l.remarks
	                          FROM globaleye.repair r,
	                               ...
	                               globaleye.part_list   pl,
	                               globaleye.code    c
	                         WHERE r.repair_id = l.repair_id(+)
	                               AND r.type_maint = c.code_id(+)
	                               and lp.asset_id = a.asset_id(+)
	                               AND lp.part_action(+) IN ('WORKED', 'REMOVED')
	                               AND a.partno_id = pl.partno_id(+)
	                               AND l.labor_id = lp.labor_id(+) ) gr,
                           (SELECT CT_ASSET_ID, GE_ASSET_ID, 
                                       SUM(DELTA) delta
                                FROM (  
                                    SELECT CM.ASSET_ID CT_ASSET_ID, 
       ...
       /*
        ...
        {
        	WriteOutput("<p>#local.sql#</p>");
        	WriteOutput("#ucase(trim(application.sessionManager.getSubSection()))#");
        	dumpLocation = expandPath("SQL_dbutilsSearchTcto.html");
			WriteDump(var="#local.qryResult#", format='html', output=dumpLocation);
        }
        catch(Any e)
        {
   ...
   public query function getEventbyAssetStartDate(required string assetId, required string maintStart, required string pmiType){
   var local = {};
       ...
       local.qryResult = "";
        ...
        local.sql = 
           "select event.event_id, event.job_no, event.asset_id, event.start_job
            from globaleye.event,globaleye.repair, globaleye.asset_inspection
      		where repair.event_id = event.event_id
			and asset_inspection.repair_id = repair.repair_id
            and event.asset_id = :asset
            and event.start_job = :mStart
            and asset_inspection.pmi_type = :pmiType";
                      
       local.qry.addParam(name="asset", value="#ucase(trim(arguments.assetId))#",cfsqltype="CF_SQL_VARCHAR");
       local.qry.addParam(name="mStart", value="#ucase(trim(arguments.maintStart))#",cfsqltype="CF_SQL_VARCHAR"); 
       local.qry.addParam(name="pmiType", value="#ucase(trim(arguments.pmiType))#",cfsqltype="CF_SQL_VARCHAR");             
   ...
   public query function searchPartOrderByLoc(required string program,string criteria="", required string loc){
    var local = {};
       ...
       local.regExCriteria = (ARGUMENTS.criteria neq "")?Replace(UCASE(ARGUMENTS.criteria)," ", "|", "All" ):"$|";
        ...
        local.sql = 
           "SELECT GCL.CODE_VALUE UNIT, GCL2.CODE_VALUE BASE, PL.NOUN REM_NOUN, PL.PARTNO REM_PARTNO, A.SERNO REM_SERNO, A.ASSET_ID REM_ASSET, NVL(A.SHIPPER, S.REM_SHIPPER) AS REQ_SHIPPER, NVL(A.TCN, S.REM_TCN) AS REQ_TCN, NVL(A.SHIP_DATE, S.REM_SRU_SHIP_DATE) AS REQ_SHIP_DATE, 
   ...
   public query function getPartsOrderedByJob(required string program, string partNoList, string serNoList, required string loc){
    var local = {};

       
       local.qry = new Query();
       local.qry.setName("partsOnOrder");
       local.qry.setDatasource(variables.instance.datasource.getDsName());
       local.qryResult = "";
        ...
        local.sql = 
           "SELECT GCL.CODE_VALUE UNIT, GCL2.CODE_VALUE BASE, PL.NOUN REM_NOUN, PL.PARTNO REM_PARTNO, A.SERNO REM_SERNO, A.ASSET_ID REM_ASSET, 
            ...
            AND S.ASSET_ID = REPA.ASSET_ID(+)
            AND REPA.PARTNO_ID = REPP.PARTNO_ID(+)
            AND PL.PARTNO IN (:partNoList)
			AND A.SERNO IN (:serNoList)
            ORDER BY S.INS_DATE desc";
                        
       local.qry.addParam(name="program", value="%#ucase(trim(arguments.program))#%",cfsqltype="CF_SQL_VARCHAR");
       local.qry.addParam(name="loc", value="#val(trim(arguments.loc))#",cfsqltype="CF_SQL_NUMERIC");  
       local.qry.addParam(name="partNoList", value="#trim(arguments.partNoList)#,0",cfsqltype="CF_SQL_VARCHAR",list="yes");   
       local.qry.addParam(name="serNoList", value="#trim(arguments.serNoList)#,0",cfsqltype="CF_SQL_VARCHAR",list="yes");   
  ...
  public query function getBenchHrsByAssetId(required string assetId, string program="ACTS"){
   ...
   public any function isDepotLocation(required string loc_id){
   	var local = {};
   	
   	local.qry = new Query();
   	local.qry.setName("qGetDepotLocations");
   	local.qry.setDatasource(variables.instance.datasource.getDsName());
   	local.qryResult = "";
   	
   	local.sql = "SELECT * FROM LOC_SET
				 WHERE SET_NAME LIKE '%DEPOT%'
				 AND LOC_ID = :loc_id ";
					                        
	   local.qry.addParam(name="loc_id", value="#trim(arguments.loc_id)#", cfsqltype="CF_SQL_VARCHAR");		           
       local.qry.setSQL(local.sql);
       local.qryResult = local.qry.execute().getResult();
    ...
    public query function getTrackingLinks(Required String shipper){
   	
   		var qry = new Query();
   		qry.setName("qTrackingLinks");
   		qry.setDatasource(variables.instance.datasource.getDsName());
   		var qryResult = "";
   		
   		var sql = "SELECT DESCRIPTION
					FROM CODE
					WHERE CODE_TYPE = 'TRACKING'
					AND CODE_VALUE = :shipper ";
					
		local.qry.addParam(name="shipper", value="#trim(arguments.shipper)#", cfsqltype="CF_SQL_VARCHAR");						
		qry.setSQL(sql);		
		results = qry.execute().getResult();
