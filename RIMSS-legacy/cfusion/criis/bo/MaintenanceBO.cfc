...
component  displayname="MaintenanceBO" hint="Maintenance Business Object" accessors="true" {
    ...
    /* Update Event */
    public Event function updateEventNOETM(required Event event, required any asset) {
        var eventService = application.objectFactory.create("EventService");	
        var assetService = application.objectFactory.create("AssetService");                    
    ...
    public Repair function createRepair(required Repair repair, required Labor labor, required LaborBitPc laborBitPc, required LaborPart workedPart, required LaborPart removedPart, required LaborPart installedPart) {
		...
		The specific HOW MAL codes to be exempt from the current process are: 
		 799 - No Defect, 
		 800 - No Defect, 
		 Removed to Facilitate Other Maintenance, 
		 804 - No Defect, 
		 removed for Scheduled Maintenance, 
		 806 - No Defect, 
		 Removed for Routine/Emergency/Special Reprogramming.
		
		Technical Requirement: When a part is removed and there is no defect, as indicated by HOW_MAL code, do not change the status of the spare asset and do not push a Depot work order. The specific HOW MAL codes to be exempt from the current process are: 799 - No Defect, 800 - No Defect, Removed to Facilitate Other Maintenance, 804 - No Defect, removed for Scheduled Maintenance, 806 - No Defect, Removed for Routine/Emergency/Special Reprogramming. An Action taken of ‘T’ should also prevent the status of the spare from changing and prevent a Depot work order. Conditional logic will have to be applied around these actions with the aforementioned criteria to achieve the desired functionality.
		*/
        ...
        transaction {
            try {
                ...
                if(arguments.removedPart.getAssetId() NEQ ""){
					...
					local.nhaAssetId = local.removedAsset.getNHAAssetID();
                	...
                	// TODO: IF REMOVED ASSET IS AN ECU,  CREATE TB 365 PMI ON ECU ASSET
                	// If the removed part is an ECU, the create a TB 365 PMI on the ECU asset
                	if(local.part.getPartno() EQ "987-1404-550"){
        						...
        						local.newJobIdService = application.objectFactory.create("NewJobIdService");        						       						
                       		    ...
                       		    local.newEvent = new Event();
                       		    	local.newEvent.setAssetId(arguments.removedPart.getAssetId());
        							...
        							local.newEvent.setLocId(application.sessionManager.getLocIdSetting());
        							local.newEvent.setSourceCat("I");
        							local.newEvent.setValid("Y");
        							local.newEvent.setOldJob(local.newJobId);
        							
        							
        							if(len(trim(arguments.repair.getStartDate())))
        								local.newEvent.setStartJob(DateAdd("d", 365, arguments.repair.getStartDate()));
        							else
        								local.newEvent.setStartJob(DateAdd("d", 365, Now()));
        							    
        							
        						local.newRepair = new Repair();
        							...
        							local.newRepair.setEtiChange("N");
        							local.newRepair.setAssetId(arguments.removedPart.getAssetId());
        							local.typeMaintCode = local.codeService.findByCodeTypeCodeValue("TYPE_MAINT", "P");
        							local.newRepair.setTypeMaint(typeMaintCode.getCodeId());
        							local.howMalCode = local.codeService.findByCodeTypeCodeValue("PMI_HOW_MAL", "999-22");
        							local.newRepair.setHowMal(local.howMalCode.getCodeId());
        							
        							     							
        							
        						local.newLabor = new Labor();	
							        ...
							        local.newLabor.setValid('Y');
							        
							        local.newLabor.setChgBy(application.sessionManager.getUserName());
							        local.newLabor.setChgDate(Now());
							        local.newLabor.setAssetId(arguments.removedPart.getAssetId());
							        	 	
        							     						
        						local.newAssetInspection = new AssetInspection();
        							local.newAssetInspection.setAssetId(arguments.removedPart.getAssetId());
        						    ...
        						    local.pmiCode = local.codeService.findByCodeTypeCodeValue("PMI_TYPE", "365-DAY");
        						    local.newAssetInspection.setPmiType(local.pmiCode.getCodeId());
        						    
        						try {
        							insertNextPmi(local.newEvent, local.newRepair, local.newLabor, local.newAssetInspection);
                ...
                if(arguments.installedPart.getAssetId() NEQ ""){
				    ...
				    local.installedPart = local.partService.getPartList(local.installedAsset.getPartnoId());
				    
				    // TODO: IF INSTALLED ASSET IS AN ECU, FIND TB 365 PMI AND END
				    if(local.installedPart.getPartno() EQ "987-1404-550"){
				    	
				    	
						local.installedAssetInspectionQry = dbUtils.getOpen365DayAssetInspectionsByAssetId(arguments.installedPart.getAssetId());
						
						if (local.installedAssetInspectionQry.recordcount GT 0) {
	            			for(i = 1; i LTE local.installedAssetInspectionQry.recordcount; i = i + 1) {
	    						
	    						// Get the Asset Inspection Obj 
	    						local.installedAssetInspection = assetInspectionService.getAssetInspection(local.installedAssetInspectionQry.HIST_ID[i]);
	    						
	    							
	    						local.installedAssetInspection.setCompleteDate(Now());
								local.assetInspectionService.updateAssetInspection(local.installedAssetInspection);
	    						
								local.installedRepair = repairService.getRepair(local.installedAssetInspection.getRepairId());								
								local.installedRepair.setStopDate(Now());
								local.repairService.updateRepair(local.installedRepair);
								
						    	local.installedEvent = eventService.getEvent(local.installedRepair.getEventId());
						    	local.installedEvent.setStopJob(Now());
						    	local.eventService.updateEvent(local.installedEvent);
						    	
						    	
	    					}
            			}
            		
            			// TODO: CHECK TO SEE IF POD HAS PMI365 RUNNING, START IF NOT
            			local.podAssetInspectionQry = dbUtils.getOpen365DayAssetInspectionsByAssetId(local.event.getAssetId());
            			if(local.podAssetInspectionQry.recordcount EQ 0){
            				...
            				local.newJobIdService = application.objectFactory.create("NewJobIdService");        						       						
                   		    ...
                   		    local.newEvent = new Event();
                   		    	local.newEvent.setAssetId(local.event.getAssetId());
    							local.newEvent.setInsBy(application.sessionManager.getUserName());
    							local.newEvent.setInsDate(Now());
    							local.newEvent.setJobNo(local.newJobId);
    							local.newEvent.setLocId(application.sessionManager.getLocIdSetting());
    						...
    						local.newRepair = new Repair();
    							...
    							local.newRepair.setOldJob(local.newJobId);
    							local.newRepair.setEtiChange("N");
    							local.newRepair.setAssetId(local.event.getAssetId());
    							local.typeMaintCode = local.codeService.findByCodeTypeCodeValue("TYPE_MAINT", "P");
    							local.newRepair.setTypeMaint(typeMaintCode.getCodeId());
    							local.howMalCode = local.codeService.findByCodeTypeCodeValue("PMI_HOW_MAL", "999-21");
    							local.newRepair.setHowMal(local.howMalCode.getCodeId());
    						...
    						local.newLabor = new Labor();	
						        ...
						        local.newLabor.setChgBy(application.sessionManager.getUserName());
						        local.newLabor.setChgDate(Now());
						        local.newLabor.setAssetId(local.event.getAssetId());
						        	 	
    							     						
    						local.newAssetInspection = new AssetInspection();
    							local.newAssetInspection.setAssetId(local.event.getAssetId());
    						    local.newAssetInspection.setInsBy(application.sessionManager.getUserName());
    						    local.newAssetInspection.setInsDate(Now());
    						    local.newAssetInspection.setWucCd("0");
    						    local.newAssetInspection.setValid("Y");
    						    
    						    local.pmiCode = local.codeService.findByCodeTypeCodeValue("INSPECTION_TYPE", "BAT365");
    						    local.newAssetInspection.setPmiType(local.pmiCode.getCodeId());
    ...
    private AssetInspection function populateAssetInspection(required any asset, required any repair) {
    	var assetInspection = new AssetInspection();
    	
    	if (arguments.asset.getAssetId() NEQ "") {
    		local.assetInspection.setAssetId(arguments.asset.getAssetId());
    	}
    	if (arguments.asset.getRepairId() NEQ "") {
    		local.assetInspection.setRepairId(arguments.asset.getRepairId());
    	}
    	
    	if (arguments.asset.getPmiType() NEQ "") {
    		local.assetInspection.setPmiType(arguments.asset.getPmiType());
    	}
    	if (arguments.asset.getWucCd() NEQ "") {
    		local.assetInspection.setWucCd(arguments.asset.getWucCd());
    	}
    	if (arguments.asset.getCompleteDate() NEQ "") {
            local.assetInspection.setCompleteDate(arguments.asset.getCompleteDate());
        }
    ...
    public Repair function updateRepair(required Repair repair, required Labor labor, required LaborBitPc laborBitPc, required LaborPart workedPart, required LaborPart removedPart, required LaborPart installedPart) {
        ...
        transaction {
            try {
                ...
                local.removedAssetInspectionQry = dbUtils.getOpen365DayAssetInspectionsByAssetId(arguments.removedPart.getAssetId());   
                // CREATE PMI FOR REMOVED PART ONLY IF REMOVED FOR THE FIRST TIME (I.E.: NOT UPON SUBSEQUENT UPDATES)             
                if(arguments.removedPart.getAssetId() NEQ "" AND local.removedAssetInspectionQry.recordcount EQ 0){
                   local.removedAsset = local.assetService.getAsset(arguments.removedPart.getAssetId());
				    local.removedPart = local.partService.getPartList(local.removedAsset.getPartnoId());
				    
                	// TODO: IF REMOVED ASSET IS AN ECU,  CREATE TB 365 PMI ON ECU ASSET
                	// If the removed part is an ECU, the create a TB 365 PMI on the ECU asset
                	if(local.removedPart.getPartno() EQ "987-1404-550"){
                ...
                if(arguments.installedPart.getAssetId() NEQ "" AND StructKeyExists(local, "installedAsset")){
