...
component  hint="Controller for processes" output="false" extends="cfc.utils.Proxy"
{
    ...
    public any function editSortie(required any sortieId){
        ...
        if(isDefined("rc.httpReferer") and len(trim(rc.httpReferer))){
            ...
            try{
                ...
                addToFormRequest("sortieId",local.sortie);
                populateSortieFromId(val(local.sortie));    
    ...
    public any function searchSortie(required string serno, string missionid, string sortieDate, required numeric programId,required numeric locId, string isNonPodded){
        var local = {};
        local.utils = new utilities();
        ...
        writeLog(file="SortieAssociate" text="Serno (#arguments.serno#) - Mission (#arguments.missionid#) - SortieDate (#arguments.sortieDate#) - ProgramId (#arguments.programId#) - Location (#arguments.locId#)");
        try{
            
            local.locationId = 0;
            
            if(!len(trim(arguments.serno)) AND ARGUMENTS.isNonPodded EQ 'N'){
			...
			if(ARGUMENTS.isNonPodded EQ 'N')
            	local.search = getDBUtils().searchSortie(argumentCollection=arguments);
            else
            	local.search = getDBUtils().searchNonPoddedSortie(argumentCollection=arguments);
    ...
    public any function exportSortie(required struct formRequest){
        var local = {};
        local.utils = new utilities();
       /* arguments.serno=arguments.formRequest.serno;
        arguments.missionid=arguments.formRequest.missionid;
        arguments.sortieDate="";
        arguments.programId=0;
        arguments.locId=0;
        arguments.exportType="";*/
        
        try{
           
            
            ARGUMENTS.formRequest.programId = getSessionFacade().getProgramIdSetting();
            ARGUMENTS.formRequest.locId = getSessionFacade().getLocIdSetting();
            
            local.page = getRootPath() & "/" & getProgram() & "/sortie/exportSortie.cfm";   
           
            if(Structkeyexists(ARGUMENTS.formRequest,"sortieCriteria")){ 
            	getSessionFacade().setValue("sortieCriteria",trim(ARGUMENTS.formRequest.sortieCriteria));
                local.search = getDBUtils().searchSortieMenu(argumentCollection=arguments.formRequest);	
                
            }else{
            	getSessionFacade().removeValue("sortieCriteria");
            	
            	if(!len(trim(arguments.formRequest.serno))){
	                throw("Serial Number must be filled in", "EmptySerialNumber");  
	            }
	            getSessionFacade().setValue("sortieSearch",ucase(trim(ARGUMENTS.formRequest.serno)));
	            
            	local.search = getDBUtils().searchSortie(argumentCollection=arguments.formRequest);
            }
            
            //local.search = getDBUtils().searchSortie(argumentCollection=arguments);
            
            addToFormRequest("sortieResults",local.search);
            addToFormRequest("exportType",arguments.formRequest.exportType);
    ...
    private void function populateSortieFromId(required numeric sortieId){
        var local = {};
        local.sortie = ARGUMENTS.sortieId;
       if(isNumeric(local.sortie) and local.sortie > 0){
           ...
           addToFormRequest("assignedUnitCd",local.sortieObj.getAssignedUnit());
           try{
               if(isNumeric(local.sortieObj.getAssignedUnit())){
                 local.code = getCodeService().getCode(val(local.sortieObj.getAssignedUnit())); 
                 addToFormRequest("assignedUnit",local.code.getCodeValue());
           ...
           addToFormRequest("isNonPodded",local.sortieObj.getIsNonPodded());   
           addToFormRequest("isDebrief",local.sortieObj.getIsDebrief());   
           addToFormRequest("isLiveMonitor",local.sortieObj.getIsLiveMonitor());    
        }   
    }
    ...
    private Sorties function populateSortie(required struct formRequest) {
        ...
        if (StructKeyExists(arguments.formRequest, "sortieId")) {
            local.sortie.setSortieId(arguments.formRequest["sortieId"]);
        }
        ...
        if (StructKeyExists(arguments.formRequest, "assignedUnitCd")) {
            local.sortie.setAssignedUnit(arguments.formRequest["assignedUnitCd"]);
        }
        
        if (StructKeyExists(arguments.formRequest, "isNonPodded")) {
            local.sortie.setIsNonPodded(arguments.formRequest["isNonPodded"]);
        }
        
        if (StructKeyExists(arguments.formRequest, "isDebrief")) {
            local.sortie.setIsDebrief(arguments.formRequest["isDebrief"]);
        }
        
        if (StructKeyExists(arguments.formRequest, "isLiveMonitor")) {
            local.sortie.setIsLiveMonitor(arguments.formRequest["isLiveMonitor"]);
        }
