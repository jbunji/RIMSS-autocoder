...
component hint="Controller for spares processes" output="false" extends="cfc.utils.Proxy"
{
    ...
    public any function getSpares( string sparesNoun= ""){
        ...
        addToRequest("spares",local.spares);
     ...
     public any function exportSpares(required struct formRequest){
       ...
       /* local.spares1 = getDBUtils().getSpares1(UCASE(TRIM(getSessionFacade().getProgramSetting())),getSessionFacade().getValue("spareNouns") );*/
        if(len(trim(getSessionFacade().getValue("spareNouns")))){
			local.spares = getDBUtils().getSpares(UCASE(TRIM(getSessionFacade().getProgramSetting())),getSessionFacade().getValue("spareNouns") );
		}else{
			local.spares = getDBUtils().searchSpares(getSessionFacade().getProgramSetting(),ARGUMENTS.formRequest.sparesCriteria);
		} 
        ...
        addToRequest("qSpares",new QueryIterator(local.spares));
         /* addToRequest("qSpares1",new QueryIterator(local.spares1));*/
    ...
    public any function searchSpares( string sparesCriteria= ""){
        ...
        getJavaLoggerProxy().fine(message="Querying Spares dbUtils.searchSpares(#getSessionFacade().getProgramSetting()#,#ARGUMENTS.sparesCriteria#)",sourceClass=getUtilities().getComponentName(this), methodName="searchSpares");
