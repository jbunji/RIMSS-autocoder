...
<cfif IsDefined("form.dir")>
	...
	<cfform name="objForm" method="post" action="uploader.cfm"> 
        ...
        <cflog file="RIMSS" text="uploader.cfm:: #path#">
      	<cftry>
      		<cffile action="upload" filefield="dir" destination="#path#" nameconflict="overwrite" accept="image/*, application/msword, application/pdf, application/vnd.ms-excel,application/vnd.openxmlfor ">
			<cfcatch>
				<cfif FindNoCase("not accepted", cfcatch.Message)>
            		<script>                	
                    	alert("Only the following file types are allowed: .jpg, .doc, .pdf");
        ...
        <cfinput type="hidden" name="fileName" id="fileName" value="#EncodeForHTML(fileSz.name)#" />
...
<cfelse>
    <form name="obj" enctype="multipart/form-data" method="post" action="uploader.cfm">
		...
		<table border="0" align="center">
            ...
            <tr>
            	<td><font color="#FF0000" size="-4">Allowable formats: (JPG, PDF, DOC)</font></td>
