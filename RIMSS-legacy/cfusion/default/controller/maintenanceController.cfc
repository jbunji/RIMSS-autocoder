import default.bo.MaintenanceBO;
...
component extends="cfc.utils.Proxy" {
    ...
    public void function insertEvent(required struct formRequest) {
        ...
        try {
	        ...
	        local.result = local.maintenanceBo.createEvent(local.event, local.cfgMeters, local.meterHist);
    ...
    public any function insertDepotEvent (required string asset, required string loc, Repair repair) {
    ...
    public any function deleteEvent( string eventJob= "", string page = ""){
        ...
        transaction{
        	try{
        		...
        		if (local.asset.getAssetType() EQ 'POD' AND local.cfgMeter.recordcount GT 0){
    ...
    public void function exportPmi(required struct formRequest){
        ...
        local.search = getDBUtils().searchPmi(getSessionFacade().getProgramSetting(),getSessionFacade().getValue("searchCriteria"),application.sessionManager.getLocIdSetting());
        		...
        		if(arguments.formRequest.assetInsId NEQ "")
        		{
        			...
        			if ((local.pmi.getNextDueDate() NEQ "") or (local.pmi.getNextDueEtm() NEQ "")) {
        					...
        					if (local.pmi.getNextDueDate() NEQ "") {
        						local.nextPmiJobExist = dbUtils.getEventbyAssetStartDate(local.pmi.getAssetId(), local.pmi.getNextDueDate());
        					} else {
    ...
    remote any function partOrderedAcknowledged(string  orderId){
    	...
    	try {
    		partOrderAcknowledge = local.maintenanceBo.acknowledgePartOrder(arguments.orderId);
 	    } catch (any e) {
            /* TODO pass error back to the createMaintenance.cfm */
            getJavaLoggerProxy().warning(message="#getUtilities().getErrorMessage(e)#",sourceClass=getUtilities().getComponentName(this), methodName="insertRepair", user=application.sessionManager.getUsername());
            addToRequest("error", {message="Update failed #e.message#"});
    	...
    	try
			{
				...
				partOrderShip = local.maintenanceBo.shipPartOrder(arguments.assetId, arguments.shipper, arguments.tcn, arguments.shipDate);
			}  catch (any e) {
            /* TODO pass error back to the createMaintenance.cfm */
            getJavaLoggerProxy().warning(message="#getUtilities().getErrorMessage(e)#",sourceClass=getUtilities().getComponentName(this), methodName="insertRepair", user=application.sessionManager.getUsername());
            addToRequest("error", {message="Update failed #e.message#"});
   ...
   private Event function populateDepotEvent(required string asset, required string loc, string eventId) {
    ...
    public any function searchPmi (string searchCriteria=""){
    	var local={};
        ...
        try{
        local.search = getDBUtils().searchPmi(getSessionFacade().getProgramSetting(),ARGUMENTS.searchCriteria,application.sessionManager.getLocIdSetting());
        }catch(any e){
        	local={};
        }
        ...
        getJavaLoggerProxy().fine(message="Querying PMI dbUtils.searchPmi(#getSessionFacade().getProgramSetting()#,#ARGUMENTS.searchCriteria#)",sourceClass=getUtilities().getComponentName(this), methodName="searchPmi");
