<cfsilent>
    ...
    <cfset headerPath = getDirectoryFromPath(getCurrentTemplatePath()) & "header.cfm"/>
	<cfset footerPath = getDirectoryFromPath(getCurrentTemplatePath()) & "footer.cfm"/>
    <cfset headerFile = "header.cfm"/>
	<cfset footerFile = "footer.cfm"/>

    <cfset program = mapProgram(lcase(trim(application.sessionmanager.getProgramSetting())))/>
    
    <cfif len(trim(program))>
	   <cfset newHeaderPath = getDirectoryFromPath(ExpandPath("/" & program & "/")) & "layout/header.cfm"/>
       <cfset newFooterPath = getDirectoryFromPath(ExpandPath("/" & program & "/")) & "layout/footer.cfm"/> 
       <cfif FileExists(newHeaderPath)>
	   	   <cfset headerPath = newHeaderPath/>
            <cfset headerFile = "/" & program & "/layout/header.cfm"/> 
			
       </cfif> 
       
       <cfif FileExists(newFooterPath)>
	   	   <cfset footerPath = newFooterPath/>
            <cfset footerFile = "/" & program & "/layout/footer.cfm"/> 
       </cfif>
