					...
					<form id="searchBacklogResults" name="searchBacklogResults" method="post" action="index.cfm">
					   <table class="globalTable" id="backlogTable" cellspacing="0" cellpadding="0">
					       ...
					       <cfoutput query="QoQSearch" group="config"> 
							   <tbody>
							   		<tr>
							   			<td bgcolor="##027FD1" style="text-align:center;font-weight:bold;color:white" colspan="12">#config#</td>	
									</tr>
									<cfoutput>												
...
<cfscript>
	...
	for(i=1; i LTE QoQSearch.recordcount;i++){
	   	...
	   	table.addCell(CreateObject("java", "com.lowagie.text.pdf.PdfPCell").init(CreateObject("java","com.lowagie.text.Phrase").init(DateFormat(encodeForHTML(QoQSearch.insDate[i]),"dd-mmm-yyyy"), body)));
