...
component  hint="Controller for Configuration processes" output="false" extends="cfc.utils.Proxy"
{
    ...
    remote any function newConfig(Required String assetId){
    	var local = {};
	...
	public function editConfig(Required String assetId){
	...
	public function addSubConfig(required String assetId){
		var local = {};
	...
	public function editSubConfig(required String assetId){
		var local = {};
	...
	public any function updateConfig(Required String assetid, Required String nhaassetid , Required noun){
			...
			try{
				...
				if(isNumeric(ARGUMENTS.assetId) && ARGUMENTS.assetId > 0){
					...
					if(isNumeric(ARGUMENTS.nhaassetid) && ARGUMENTS.nhaassetid > 0){
						...
						//editConfig(ARGUMENTS.assetid);rc.topAssetID
						if(StructKeyExists(rc, "LRU") and rc.LRU){
							editConfig(ARGUMENTS.nhaassetid);
						}else{
	...
	public any function moveConfig(Required String assetId, Required String newassetId){
	...
	public void function exportConfig(required struct formRequest){
		...
		var local = {};
       	...
       	local.aid = rc.util.decryptId(ARGUMENTS.formRequest.assetID);
		local.qConfigs = getDBUtils().readByNhaAssetId(local.aid); 
		local.qConfigs = rc.util.addEncryptedColumn(local.qConfigs,'asset_id');		
		...
		addToRequest("qSearch",local.qSearch);
		addToRequest("qConfigs",local.qConfigs);
