...
async function main() {
  ...
  const distinctSysTypes = await prisma.$queryRaw`
    ...
    FROM cfg_set c
    LEFT JOIN partno p ON c.partno_id = p.partno_id
    WHERE c.pgm_id = 1
    GROUP BY p.sys_type
  ...
  console.log('\nConfigurations with ECU system type:');
  console.log(ecuConfigs);

  // Check what the Part model actually looks like
  const sampleParts = await prisma.partno.findMany({
    where: { pgm_id: 1 },
