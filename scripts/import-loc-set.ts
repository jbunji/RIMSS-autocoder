...
// Program code to pgm_id mapping (from database)
const PROGRAM_MAPPINGS: Record<string, number> = {
  'ACTS': 2,
  'CRIIS': 1,
  'ARDS': 3,
  '236': 4,
...
async function main() {
  ...
  // Path to the CSV file - relative to backend folder when run with `cd backend && npx tsx ../scripts/import-loc-set.ts`
  ...
  // Get location ID mapping (Oracle -> PostgreSQL) - relative to backend folder
  const idMappingsPath = path.resolve(process.cwd(), 'id-mappings-v2.json')
